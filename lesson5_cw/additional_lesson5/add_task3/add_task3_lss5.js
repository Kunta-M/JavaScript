...
for (const user of users) {
    addressInfo.push(JSON.parse(JSON.stringify(user.address)))
}
...
for (const users3Element of users3) {
    ...
    let address = ``;
    for (const addressElement in users3Element.address) {
        address = address + "  " + users3Element.address[addressElement];
    }

    h2.innerText = users3Element.name;
    p1.innerText = users3Element.age;
...
for (const users4Element of users4) {
    let div = document.createElement('div');
    ...
    for (const addressElement in users4Element.address){
        let items = document.createElement('p');
        items.innerText = users4Element.address[addressElement];
        address.appendChild(items)
    }

    name.innerText = users4Element.name;
